...
iappend(uint inum, void *xp, int n)
{
  ...
  while(n > 0){
    ...
    bcopy(p, buf + off - (fbn * 512), n1);
       ...
       //direct block number
    if(fbn< NDIRECT){
      din.checksum[fbn] =  adler32(buf, BSIZE);
     // uint gg = din.checksum[fbn];
     // printf("filling checksum in direct blocks : %d\n",gg);
    }
    
    if(fbn >= NDIRECT && fbn < MAXFILE)
    {
     
        rsect(xint(din.addrs[NDIRECT]), (char*)indirect);
    	indirect[fbn - NDIRECT + 64] = adler32(buf, BSIZE);
